...
for item in items:
    ...
    state = " ".join(str(x or "") for x in [
        item.get("lifecycleStatus"), item.get("status"),
        model.get("lifecycleStatus"), model.get("status")
    ]).lower()
    ...
    if isinstance(caps, dict) and caps:
        chat_flags = [caps.get("chatCompletion"), caps.get("chat_completions"), caps.get("responses")]
        if all(flag is False for flag in chat_flags if flag is not None):
            continue

    priority = 50
    for idx, prefix in enumerate([
        "gpt-5-mini", "gpt-5-nano", "gpt-5", "gpt-4.1-mini", "gpt-4o-mini", "gpt-4.1", "gpt-4o"
    ]):
...
for _, name, version in sorted(set(rows), key=lambda x: (x[0], x[1], x[2]), reverse=False):
...
PY
  ...
  do
    ...
    for SKU in GlobalStandard DataZoneStandard Standard
    do
...
az cognitiveservices account deployment show \
  ...
  --resource-group "$RG" \
  --deployment-name "$DEPLOYMENT_NAME" \
  --query '{Deployment:name,Model:properties.model.name,Version:properties.model.version,Status:properties.provisioningState,SKU:sku.name}' \
  --output table
